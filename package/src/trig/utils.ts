...
export function piCoeff(expr: Expr): [bigint, bigint] | null {
	...
	if (l instanceof PiConstant && r instanceof Rational) return [r.numerator, r.denominator];
	if (r instanceof PiConstant && l instanceof Rational) return [l.numerator, l.denominator];
...
export function stripNeg(expr: Expr): Expr {
	...
	if (expr instanceof Mul) {
		if (expr.left instanceof Num) return new Mul(new Num(-expr.left.value), expr.right);
		if (expr.left instanceof Rational) return new Mul(expr.left.neg(), expr.right);
	}
